...
import api, { getBadgeTemplates } from '@/lib/api';
...
const createDefaultTemplate = (): BadgeTemplate => ({
  ...
  elements: [
    ...
    { id: uuidv4(), type: 'qr', vCardData: { firstName: '', lastName: '', organization: '', title: '', email: '', phone: '' }, x: 148, y: 350, width: 100, height: 100, rotation: 0, zIndex: 2 },
    { id: uuidv4(), type: 'text', content: '{guestType}', x: 20, y: 480, width: 356, height: 20, rotation: 0, zIndex: 2, fontFamily: 'Arial', fontSize: 16, fontWeight: 'bold', color: '#10B981', textAlign: 'center' },
  ],
});
    ...
    getBadgeTemplates(eventId)
      .then(res => {
        if (Array.isArray(res.data) && res.data.length > 0) {
          setTemplate({ ...res.data[0].template_json, id: res.data[0].id, name: res.data[0].name });
        } else {
